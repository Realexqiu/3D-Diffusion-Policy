...
def main():
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        window_positions = {
            ...
            "First-DT-Left": (2*DISPLAY_WIDTH, 0),
            "First-DT-Right": (2*DISPLAY_WIDTH, DISPLAY_HEIGHT),
            ...
            "Last-DT-Left": (5*DISPLAY_WIDTH, 0),
            "Last-DT-Right": (5*DISPLAY_WIDTH, DISPLAY_HEIGHT),
        }
        ...
        # Display DenseTact images (first frame)
        if dt_left_key and len(f[dt_left_key]) > 0:
            dt_left_first = f[dt_left_key][first_frame_idx]
            dt_left_display = resize_for_display(dt_left_first)
            cv2.imshow("First-DT-Left", dt_left_display)
            cv2.moveWindow("First-DT-Left", *window_positions["First-DT-Left"])
        
        if dt_right_key and len(f[dt_right_key]) > 0:
            dt_right_first = f[dt_right_key][first_frame_idx]
            dt_right_display = resize_for_display(dt_right_first)
            cv2.imshow("First-DT-Right", dt_right_display)
            cv2.moveWindow("First-DT-Right", *window_positions["First-DT-Right"])
        ...
        # Display DenseTact images (last frame)
        if dt_left_key and len(f[dt_left_key]) > 0:
            dt_left_last = f[dt_left_key][last_frame_idx]
            dt_left_display = resize_for_display(dt_left_last)
            cv2.imshow("Last-DT-Left", dt_left_display)
            cv2.moveWindow("Last-DT-Left", *window_positions["Last-DT-Left"])
        
        if dt_right_key and len(f[dt_right_key]) > 0:
            dt_right_last = f[dt_right_key][last_frame_idx]
            dt_right_display = resize_for_display(dt_right_last)
            cv2.imshow("Last-DT-Right", dt_right_display)
            cv2.moveWindow("Last-DT-Right", *window_positions["Last-DT-Right"])
